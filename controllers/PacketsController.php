...
class PacketsController extends DefaultController
{
    ...
    public function actionPacketAttributeDelete( ){

     if (Yii::$app->request->isAjax && Yii::$app->request->isPost) {
            Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;

            $model = \app\models\radius\Radgroupreply::find()
            ->where(['id' => Yii::$app->request->post('radgroup_id')])
            ->one();
    ...
    public function actionChangePacketAttribute( $id ){
        $model = \app\models\radius\Radgroupreply::find()->where(['id'=>$id])->one();
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
       
            return $this->redirect(['index']);
        }
        return $this->renderIsAjax('change-packet-attribute', [
            'model' => $model,
        ]);
    }
    ...
    public function actionValidateChangePacketAttribute(){
          $model = new \app\models\radius\Radgroupreply;
    ...
    public function actionUpdate($id){
        ...
        $oldPacketName  = $model['packet_name'];
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            if ( $model->service->service_alias == "internet" ) {
                $radgroupreplyModel = \app\models\radius\Radgroupreply::find()
                ->where(['groupname'=>$oldPacketName])
                ->all();
                foreach ($radgroupreplyModel as $rdk => $radgroupreply) {
                    \app\models\radius\Radgroupreply::updatePacket( $oldPacketName , $model->packet_name );
                }
            }
    ...
    public function actionAddAttribute( $id ){
        ...
        $packetModel = \app\models\Packets::find()->where(['id'=>$id])->one();
        $model = new \app\models\radius\Radgroupreply;
       ...
       if ($model->load(Yii::$app->request->post()) && $model->save()) {
        ...
        $radgroupreplyModel = \app\models\radius\Radgroupreply::find()->where(['groupname'=>$packetModel['packet_name']])->asArray()->all();

        return $this->renderIsAjax('add-attribute', [
            'model' => $model,
            'packetModel' => $packetModel,
            'radgroupreplyModel' => $radgroupreplyModel,
        ]);
   }
    ...
    public function actionDelete($id){
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
         $model = \app\models\Packets::find()->where(['id'=>$id])->one();

         $usageCount = \app\models\UsersServicesPackets::find()->where(['packet_id'=>$model->id])->count();
         if ( $usageCount > 0 ) {
             return ['status' => 'error','message'=>Yii::t('app','{count} people are using {packet} packet',['count'=>$usageCount,'packet'=>$model['packet_name']])];
         }

        $radgroupreplyModel = \app\models\radius\Radgroupreply::deleteAll(['groupname'=>$model['packet_name']]);
        $logMessage = $model['packet_name']." packet was deleted successfuly";
        if ($model->delete()) {
            \app\models\Logs::writeLog(Yii::$app->user->username, null, $logMessage, time());
            return ['status' => 'success','message'=>Yii::t('app','Packet was deleted successfuly')];
        }
    }
