...
use yii\db\Query;
/**
 * RadacctSearch represents the model behind the search form of `\app\models\radius\Radacct`.
 */
class RadacctSearch extends Radacct
{
    ...
    {
        return [
            [['radacctid', 'acctinterval', 'acctsessiontime'], 'integer'],
            [['acctinputoctets', 'acctoutputoctets'],'number'],
    ...
    public function search($params)
    {
        ...
        ->innerJoin(
        ['s' => $subQuery],
        'r.username = s.username AND r.radacctid = s.max_radacctid'
        )
        ...
        $cookieName = '_grid_page_size_online_users';
        ...
        $query->andFilterWhere(['like', 'acctsessionid', $this->acctsessionid])
            ->andFilterWhere(['ROUND(acctoutputoctets / (1024 * 1024), 2)' => $this->acctoutputoctets])
            ->andFilterWhere(['ROUND(acctinputoctets / (1024 * 1024), 2)' => $this->acctinputoctets])
            ...
            ->andFilterWhere(['like', 'r.username', $this->username])
