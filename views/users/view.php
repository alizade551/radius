            ...
            <?php if ($service_one->service->service_alias == "internet"): ?>   
                <div class="tab-pane p-3" id="<?=$service_one->service->service_alias ?>" role="tabpanel">
                 <?php  Pjax::begin(['id'=>'pjax-inet-table','enablePushState'=>true]);  ?>
                        ...
                        ->select('users_inet.*,address_district.nas_id as router_id,service_packets.packet_price as packet_price,users_services_packets.price as custom_price,users.fullname as user_fullname,service_packets.packet_name as packet_name,nas.server as nas,nas.nasname as router_name')
                        ...
                        ->leftJoin('nas','address_district.nas_id=nas.id')
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>  
                                    ...
                                    <td><?php if(isset( $value_inet['packet_name']  ) && $value_inet['status'] == "1" ){echo $value_inet['packet_name'];}else{echo "BLOCKED";} ?></td>
                                   
                                    <td>
                                        <?php if ( \app\models\radius\Radacct::isUserOnline( $value_inet['login'] ) ): ?>
                                    ...
                                    <?php if (User::canRoute('/users/ip-history')): ?>
                                        <td class="check-user-internet">
                                            <a  style="margin-left: -30px; display: block; text-align: center;" class="modal-d"  href="<?=$langUrl ?>/users/ip-history?login=<?=$value_inet['login'] ?>" title="<?=Yii::t("app","Radius accounting for packet {login}",['login'=>$value_inet['login']]) ?>" >
                                             <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path></svg>
                                            </a>
                                        </td>                                              
                                    <?php endif ?>  
