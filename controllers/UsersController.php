...
class UsersController extends DefaultController
{
    ...
    public function actionDisableAllTemporaryServices(){
        ...
        ->leftJoin('nas', 'nas.id=address_district.nas_id')
    ...
    public function actionAddBlackList($user_id){
       ...
       $router_model = \app\models\radius\Nas::find()
            ->where(['id' => $user_model->district->nas_id])
            ->asArray()
            ->one();
                    ...
                    foreach ($services_user_paccket as $key => $value_ser) {
                        if ($value_ser->service->service_alias == "internet") {
                            ...
                            foreach ($internet_service as $key => $serv) {
                                ...
                                \app\models\radius\Radcheck::deleteUser($serv->login);
                                \app\models\radius\Radusergroup::deleteRadUserGroup($serv->login);
                                \app\models\radius\Radacct::deleteAll(["username"=>$serv->login]);
                                \app\models\radius\Radpostauth::deleteAll(["username"=>$serv->login]);
                                ...
                                if ($serv->static_ip) {
                                    ...
                                    if ( $staticIpModel->save(false) ) {
                                        \app\models\radius\Radreply::deleteStaticIp( $serv->login,$staticIpModel->public_ip);
                                    }
                                }
                                \app\components\COA::disconnect( $serv->login );
                                $serv->delete();
                            }
                          ...
                          if( $userService->save(false) ){
                            if ( $userService->service->service_alias == "internet"  ) {
                                ...
                                if ( $inetModel->save(false) ) {
                                    \app\models\radius\Radgroupreply::unblock( $inetModel->login ,$userService->packet->packet_name );
    ...
    public function actionIpHistory( $login ){
        $model = new \app\models\radius\Radacct;
        $model->scenario = \app\models\radius\Radacct::IP_HISTORY;

        if ($model->load(Yii::$app->request->post())) {
            $query = \app\models\radius\Radacct::find()
                ->select(['framedipaddress','nasipaddress','acctstoptime','acctoutputoctets','acctinputoctets','acctsessiontime'])
                ->where(['username'=>$login])
                ->andWhere(['between', 'acctstarttime', $model->startDate, $model->endDate])
                ->groupBy('framedipaddress');

            // GridView için dataprovider oluştur
            $result = $query->asArray()->all();



        } else {
            // Varsayılan olarak tüm verileri göstermek için boş bir dataprovider oluştur
            $result = [];
        }

        return $this->renderIsAjax('ip-history', [
            'model' => $model,
            'login' => $login,
            'result' => $result,
        ]);
    ...
    public function actionCheckUserInternet($login){
        ...
        $isUserOnline = \app\models\radius\Radacct::isUserOnline( $user_inet_model['login'] );
        $getUserTrafficLastMonth = \app\models\radius\Radacct::getUserTrafficLastMonth( $user_inet_model['login'] );


        $acctsessiontime = \app\models\radius\Radacct::getAcctSessionInfoByUsername( $user_inet_model['login'] )['acctsessiontime'];
        $framedipaddress = \app\models\radius\Radacct::getAcctSessionInfoByUsername( $user_inet_model['login'] )['framedipaddress'];
        $nasipaddress = \app\models\radius\Radacct::getAcctSessionInfoByUsername( $user_inet_model['login'] )['nasipaddress'];
        $mac_address = \app\models\radius\Radacct::getAcctSessionInfoByUsername( $user_inet_model['login'] )['mac_address'];

        $download = \app\models\radius\Radacct::getAcctSessionInfoByUsername( $user_inet_model['login'] )['download'];
        $upload = \app\models\radius\Radacct::getAcctSessionInfoByUsername( $user_inet_model['login'] )['upload'];


        $montlyDownload = \app\models\radius\Radacct::getUserTrafficLastMonth( $user_inet_model['login'] )['total_download'];
        $montlyUpload = \app\models\radius\Radacct::getUserTrafficLastMonth( $user_inet_model['login'] )['total_upload'];
        
        $data = [
            'total_download'=>$download,
            'total_upload'=>$upload,
            'montlyDownload'=>$montlyDownload,
            'montlyUpload'=>$montlyUpload,
        ...
        return $this->renderIsAjax('check-internet', ['data' => $data,'user_inet_model'=>$user_inet_model]); 
    }
            ...
            if ( $model->status == 2 || $model->status == 1 ) {
                foreach ( $model->usersInets as $key => $value_inet ) {
                    if ( $value_inet->static_ip != "") {
                        ...
                        if ( $static_model->save(false) ) {
                            \app\models\radius\Radreply::deleteStaticIp($value_inet->login,$static_model->public_ip);
                        }
                        $value_inet->static_ip = "";
                    }
                    $value_inet->status = 3;
                    if ($value_inet->save(false)) {
                        \app\models\radius\Radcheck::deleteUser( $value_inet->login );
                        \app\models\radius\Radusergroup::deleteRadUserGroup( $value_inet->login );
                        \app\models\radius\Radacct::deleteAll( ['username'=>$value_inet->login] );
                        \app\models\radius\Radpostauth::deleteAll( ['username'=>$value_inet->login] );
                        \app\components\COA::disconnect( $value_inet->login );
                    }
                }
    ...
    public function actionServiceDelete(){
        if (Yii::$app->request->isAjax && Yii::$app->request->isPost) {
            ...
            if ( $model_user_ser_packets->service->service_alias == "internet" ) {
                $router_model = \app\models\radius\Nas::find()
                ->where(['id' => $model_user->district->nas_id])
                ->asArray()
                ->one();
                ...
                if ( $inet_model ) {
                        ...
                        \app\models\radius\Radcheck::deleteUser( $inet_model->login );
                        \app\models\radius\Radusergroup::deleteRadUserGroup( $inet_model->login );
                
                        if ( $inet_model->static_ip != "" ) {
                            $static_ip_M = \app\models\IpAdresses::find()->where(['id' => $inet_model->static_ip])->one();
                            $static_ip_M->status = '0';
                            $static_ip_M->save(false);
                            \app\models\radius\Radreply::deleteStaticIp( $inet_model->login, $static_ip_M['public_ip'] );
    ...
    public function actionChangePacket($id){
          ...
          if ( $model->load(Yii::$app->request->post()) && $model->validate() ) {
            ...
            if ($model->service->service_alias == "internet") {
                $routerModel = \app\models\radius\Nas::find()
                ->where(['id' => $userModel->district->nas_id])
                ->asArray()
                ->one();
            ...
            if ($model->save(false)) {
                ...
                if ($model->service->service_alias == "internet") {
                    ...
                    if ($model->status == 2 || $model->status == 3) {
                        \app\models\radius\Radgroupreply::block( $inetModel->login );
                    }else{
                    ...
                    if (empty($staticId)) {
                        if ($inetModel->static_ip != "") {
                            ...
                            if ($staticIpModel->save(false)) {
                                $inetModel->static_ip = '';
                                 \app\models\radius\Radreply::deleteStaticIp( $inetModel->login, $staticIpModel['public_ip'] );
                            }
                        }
                        ...
                        if ($inetModel->static_ip != "") {
                            ...
                            \app\models\radius\Radreply::deleteStaticIp( $inetModel->login, $staticIpQuery['public_ip'] );
                        }
                        ...
                        if ($staticIpNewQuery->save(false)) {
                            ...
                            if ($inetModel->save(false)) {
                                \app\models\radius\Radreply::addStaticIP( $inetModel->login, $findStaticIp['public_ip'] );
                                \app\components\COA::disconnect( $inetModel->login );
