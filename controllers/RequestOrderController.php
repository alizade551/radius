                ...
                if ($checkService == null) {
                    ...
                    if ($serviceModel->service_alias == "internet") {
                        ...
                        if ( isset(Yii::$app->request->post('UsersServicesPackets')['port_type']) ) {
                            ...
                            $inetModel->nas_id = $routerModel['id'];
                            $inetModel->u_s_p_i = $model->id;
                            $inetModel->connect_type = Yii::$app->request->post("UsersServicesPackets")['connect_type'];
                            ...
                            if ( $inetModel->save(false) ) {
                                if ( $model->packet_login != "" ) {
                                    $inetModel->login = $model->packet_login;
                                }else{
                                    $inetModel->login = $siteConfig['inet_ppoe_login_start'] . sprintf("%06d", $inetModel->id);
                                }
                                
                                // re-save for auto incriment id
                                    if ( $inetModel->save(false) ) {
                                        ...
                                        ->one();
                                         $addUserToRadius = \app\models\radius\Radcheck::addUser(
                                            $inetModel->login,
                                            RadiusAttributes::CLEARTEXT_PASSWORD,
                                            RadiusAttributes::OPERATION_EQUALS,
                                            $inetModel->password
                                        );

                                         if ( $addUserToRadius == true ) {
                                            $radUserGroup = \app\models\radius\Radusergroup::createRadUserGroup( $inetModel->login, $packetModel->packet_name );
                                         }
                                        ...
                                        if (!empty(Yii::$app->request->post("UsersServicesPackets")['static_ip_address'])) {
                                            ...
                                            // statik ip elave et
                                            \app\models\radius\Radreply::addStaticIP( $inetModel->login,$staticIpModel['public_ip'] );
                                        ...
                                        \app\components\COA::disconnect( $siteConfig['inet_ppoe_login_start'] . sprintf("%06d", $inetModel->id) );
                                    }
                                }
                        }
                    }
            ...
            foreach ($model_user_services_new as $key => $value_model_user_services_new) {
                if ($value_model_user_services_new->service->service_alias == "internet") {
                    ...
                    if ($inet_model->static_ip) {
                        ...
                        $staticIpModel->status = "0";
                        $staticIpModel->save(false);
                        \app\models\radius\Radreply::deleteStaticIp( $inet_model->login, $staticIpModel['public_ip'] );
                    }
                                ...
                                if ( $inetModel->save(false) ) {
                                    ...
                                    if ( $model->second_status == 4  ) {
                                        $addUserToRadius = \app\models\radius\Radcheck::addUser(
                                         ...
                                         if ( $addUserToRadius == true ) {
                                            $radUserGroup = \app\models\radius\Radusergroup::createRadUserGroup(  $inetModel->login, $inetModel->packet->packet_name );
                                         }
                                        ...
                                        \app\models\radius\Radgroupreply::unBlock( $inetModel->login, $inetModel->packet->packet_name);
                                        \app\components\COA::disconnect( $inetModel->login);
                                    }
                                }
