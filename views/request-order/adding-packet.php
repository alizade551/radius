                    ...
                    ?>
                       ...
                       <?php  Pjax::begin(['id'=>'pjax-adding-packet-form','enablePushState'=>true]);  ?>
                            ...
                            <?php if ( $serviceGet['service_alias'] == "internet" ): ?>
                                ...
                                <?= $form->field($model, 'packet_tags')->dropDownList(
                                    ...
                                    [
                                        'onchange'=>'
                                            $(".select_loader").show();
                                            $.pjax.reload({
                                                url: "'.Url::to(['adding-packet']).'?id='.Yii::$app->request->get('id').'&service_id='.Yii::$app->request->get('service_id').'&packet_tags="+$(this).val(),
                                                container: "#pjax-adding-packet-form-static-ip",
                                                timeout: 5000
                                            });
                                            $(document).on("pjax:complete", function() {
                                              $(".select_loader").hide();
                                ...
                                <?php  Pjax::begin(['id'=>'pjax-adding-packet-form-static-ip','enablePushState'=>true]);  ?>


                                 <?=$form->field($model, 'connect_type')->dropDownList(
                                    ['pppoe'=>'pppoe','ipoe'=>'ipoe'],
                                    ['prompt'=>Yii::t('app','Select'),
                                    'onchange'=>'
                                            $(".select_loader").show();
                                            $.pjax.reload({
                                                url: "'.Url::to(['adding-packet']).'?id='.Yii::$app->request->get('id').'&service_id='.Yii::$app->request->get('service_id').'&packet_tags='.Yii::$app->request->get('packet_tags').'&connect_type="+$(this).val(),
                                                container: "#pjax-adding-packet-form-connect-type",
                                                timeout: 5000
                                            });
                                            $(document).on("pjax:complete", function() {
                                              $(".select_loader").hide();
                                        ...
                                        ',]
                                )->label(Yii::t('app','Connect type'));?>


                                <?php $model->packet_id = Yii::$app->request->get('packet_tags') ?>


                                 <?php Pjax::end(); ?>  


                                  <?php  Pjax::begin(['id'=>'pjax-adding-packet-form-connect-type','enablePushState'=>true]);  ?>
                                  <?php if ( $serviceGet['service_alias'] == "internet" && Yii::$app->request->isPjax &&  Yii::$app->request->get('connect_type') == "pppoe" ): ?>
                                        <div class="n-chk" >
                                            <label class="new-control new-checkbox checkbox-primary">
                                                <input id="custom-lp"  type="checkbox" class="new-control-input" >
                                                <span class="new-control-indicator"></span><?=Yii::t('app','Define custom login & password') ?>
                                            </label>
                                        </div>

                                        <div class="lp-container">
                                            <?php $lastInetAcc = \app\models\UsersInet::find()->orderBy(['id'=>SORT_DESC])->one() ?>
                                            <?=$form->field($model, 'packet_login')->textInput(['value'=>$siteConfig['inet_ppoe_login_start'] . sprintf("%06d", ( $lastInetAcc->id +1 ) )])->label("Packet login");?>
                                            <?=$form->field($model, 'packet_password')->textInput(['value'=>\app\models\UsersServicesPackets::getRandomString()])->label();?>
                                        </div>
                                  <?php endif ?>

                                   <?php if ( $serviceGet['service_alias'] == "internet" && Yii::$app->request->isPjax &&  Yii::$app->request->get('connect_type') == "ipoe" ): ?>
                                    <?=$form->field($model, 'packet_login')->textInput()->label("Mac address");?>
                                    <?=$form->field($model, 'packet_password')->textInput(['value'=>\app\models\UsersServicesPackets::getRandomString()])->label();?>
                                   <?php endif ?>
                                    <?php 
